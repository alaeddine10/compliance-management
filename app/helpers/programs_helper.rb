module ProgramsHelper
  ...
  def program_stats(program)
    stats = {}
    stats[:sections_count] = 0 #Section.where(:program_id => program.id).count()
    stats[:sections_done_count] = 0 #Section.joins(:controls).where(:program_id => program.id).count(:distinct => true)
    stats[:sections_undone_count] = 0 #Section.where(:program_id => program.id).count() - stats[:sections_done_count]
    ...
    stats[:controls_count] = 0 #controls.count(:distinct => true)
    stats[:controls_parented_count] = 0 #controls.where(Control.arel_table[:parent_id].not_eq(nil)).count(:distinct => true)
